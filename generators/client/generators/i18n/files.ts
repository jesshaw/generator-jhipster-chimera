...
export function writeLocalizationEntityFiles() {
  return {
    async writeLocalizationEntityFiles({ application, entities }) {
      ...
      // Copy each
      const { clientSrcDir, frontendAppName, baseName } = application;
      ...
      for (const entity of entitiesToWriteTranslationFor) {
        for (const lang of languagesToApply) {
          if (!entity.builtInUserManagement) {
            ...
            entity.entityClassHumanized = entity.entityClassHumanized || entity.entityClass || 'User';
            entity.entityClassPluralHumanized = entity.entityClassPluralHumanized || entity.entityClassPlural || 'Users';
          }
          ...
          entity.fields.forEach(field => {
            if (!field.fieldNameHumanized) {
              field.fieldNameHumanized = field.fieldNameCapitalized || field.fieldName;
            }
          });

          await this.writeFiles({ sections: entityClientI18nFiles, context: { ...entity, clientSrcDir, frontendAppName, baseName, lang } });
        }
      }
